...
if (isset($_POST["assign_equipment"])) {
    ...
    if (empty($equipment)) {
        ...
        if ($assignEquipment) {
            header("Location: equipments.php?msg=Equipment has been added successfully!");
        }
    }
}
                        ...
                        ?>
                            <div class="card shadow mb-4">
                                <div class="card-header d-flex align-items-center">
                                    ...
                                    <h6 class="m-0 font-weight-bold">Update Equipments</h6>
                                </div>
                                <form action="update-equipment-action.php" method="post">
                                    <div class="card-body">
                                        <div class="form-floating">
                                            <select class="form-control" id="pwd" name="pwd_id">
                                                <?php
                                        ...
                                        <select class="form-control" id="equipment" name="equipment">
                                            <?php
                                                ...
                                                foreach ($res1 as $row) {
                                                    // Ensure `equipment_name` exists in the fetched data
                                                    if (isset($row['equipment_name']) && !in_array($row['equipment_id'], $displayedEquipment)) {
                                                        $selected = ((int)$row['equipment_id'] === (int)$fetchEquipment) ? 'selected' : '';
                                                        $displayedEquipment[] = $row['equipment_id']; // Mark as displayed
                                            ?>
                                                        <option value="<?= htmlspecialchars($row['equipment_id']) ?>" <?= $selected ?>>
                                        ...
                                        <button class="w-100 btn btn-lg btn-primary my-3" type="submit" name="update">Update Equipment</button>
                                    </div>
                                </form>
                            </div>
                        <?php } ?>
